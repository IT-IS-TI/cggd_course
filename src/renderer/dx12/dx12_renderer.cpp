...
void cg::renderer::dx12_renderer::init()
{
	...
	cb.light.position = float4{
		settings->camera_position[0],
		settings->camera_position[1] + 20.f,
		settings->camera_position[2] - 5.f, 1.f
	};
...
void cg::renderer::dx12_renderer::create_pso(const std::string& shader_name)
{
	...
	auto pixel_shader_texture = compile_shader(get_shader_path(shader_name), "PSMain_texture", "ps_5_0");
	auto vertex_shader_shadow_map = compile_shader(get_shader_path(shader_name), "VSShadowMap", "vs_5_0");
...
void cg::renderer::dx12_renderer::load_assets()
{
	...
	cbv_srv_heap.create_heap(device, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, static_cast<UINT>(3 + texture_num), D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE);
...
void cg::renderer::dx12_renderer::populate_command_list()
{
	...
	command_list->ClearDepthStencilView(dsv_heap.get_cpu_descriptor_handle(), D3D12_CLEAR_FLAG_DEPTH, 
		1.0f, 0, 0, nullptr);
	...
	command_list->OMSetRenderTargets(1, &rtv_heap.get_cpu_descriptor_handle(frame_index), FALSE, 
		&dsv_heap.get_cpu_descriptor_handle());
	...
	for (size_t s = 0; s < model->get_index_buffers().size(); s++)
	{
		...
		command_list->SetGraphicsRootDescriptorTable(1, cbv_srv_heap.get_gpu_descriptor_handle(static_cast<UINT>(s + 3)));
		command_list->IASetVertexBuffers(0, 1, &vertex_buffer_views[s]);
		command_list->IASetIndexBuffer(&index_buffer_views[s]);
		command_list->DrawIndexedInstanced(static_cast<UINT>(model->get_index_buffers()[s]->count()), 1, 0, 0, 0);
	}
